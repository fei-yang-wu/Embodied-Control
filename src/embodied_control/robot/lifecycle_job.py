...
class RealtimeSpec(JobModel):
    ...
    # Where the policy and the reference encoder run. `cuda` (default since
    # 2026-09-13) cuts the inference share of the control tick from ~1.4 ms
    # to ~0.15 ms on the deployment host and reproduces the golden trace to
    # 1e-6 like `cpu`, the reference path. `tensorrt` is another 0.05 ms
    # faster but its TF32 engines miss the encoder tolerance (8e-4), so it is
    # opt-in and needs the `native-gpu` environment. Part of the rehearsal
    # identity: hardware runs what the plant ran.
    inference_provider: Literal["cpu", "cuda", "tensorrt"] = "cuda"
