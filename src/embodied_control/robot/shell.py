...
def build_session_bindings(session) -> list[KeyBinding]:
    ...
    return [
        ...
        KeyBinding("g", "arm: blend in, policy stands on frame 0", lambda: report(session.arm()), "episode"),
        KeyBinding("G", "play: release the reference, motion starts", lambda: report(session.play()), "episode"),
...
def build_lifecycle_bindings(lifecycle) -> list[KeyBinding]:
    ...
    return [
        ...
        KeyBinding("g", "arm: blend in, policy stands on frame 0", lambda: report(lifecycle.arm()), "episode"),
        KeyBinding("G", "play: release the reference, motion starts", lambda: report(lifecycle.play()), "episode"),
