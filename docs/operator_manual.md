...
## Start the planner yourself

The planner is a separate process that loads its own checkpoint: the
VLA one is several gigabytes on the GPU. Nothing starts it for you.
Press `p` to launch it for the current selection, then `r` to build
the tracker against its mailboxes. A lifecycle key pressed before
that says so rather than launching one. Pass `--planner-autostart` to
couple them again.
